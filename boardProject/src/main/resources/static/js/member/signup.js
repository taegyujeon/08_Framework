...
// 인증 번호 입력 input
constauthKey = document.querySelector("#authKey")
...
checkAuthKeyBtn.addEventListener("click",()=>{
  ...
  if(authKey.value.length < 6){ // 인증번호가 제대로 입력이 안된 경우
    alert("인증번호를 정확히 입력해주세요.");
    return;
  }

  const obj = {"email" : memberEmail.value,
